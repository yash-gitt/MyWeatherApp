...
// fetch jason return promise thata why we use await and async function
...
const tempCard = (val, unit="cel") => {
    ...
    <h6 class="card-subtitle mt-2  ${unit}">${val.temp}</h6>
...
const displayWeather = (data) => {
    ...
    <div class="card-body">
        <h5 class="card-title">${data.name}, ${data.sys.country} <span class="float-end units"><a href="#" class="unitlink active" data-unit="cel">°C</a> |
         <a href="#" data-unit="far" class="unitlink">°F</a></span></h5>
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (countries) {
        countriesOptions += `<option values="">Country</option>`;
    ...
    countriesListDropDown.addEventListener("change", async function () {
        ...
        let statesOptions = "";
        if (states) {
            statesOptions += `<option values="">state</option>`;
    ...
    statesListDropDown.addEventListener("change", async function () {
        ...
        let citiesOptions = "";
        if (cities) {
            citiesOptions += `<option values="">city</option>`;
            cities.forEach((city) => {
    ...
    document.addEventListener("click", async (e) => {
        if (e.target.classList.contains("unitlink")) {
            ...
            const unitFalg = unitValue == "far" ? "imperial" : "metric";
